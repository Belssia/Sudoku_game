...
public class UserManagementServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("create") != null) {
			LOGGER.debug("Redirection vers le formulaire de cr√©ation des utilisateurs");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (gameContext.getUserByLogin(login) != null) {
			...
			messages.add(new Message("Login existe deja†", Message.WARN));
		...
		// On redirige vers la page login avec un message de succ√®s
		messages.add(new Message("Utilisateur est correctement ajoutÈ", Message.INFO));
